...
- **Agent 无内核级保活**：不启用 TCP keepalive（内核默认 2 小时且需显式开启，对监控无意义），也不设置 `set_nodelay`；连接韧性靠应用层超时——连接建立 15 秒总超时（DNS/TCP/TLS/HTTP 101）、控制通道单帧发送 10 秒超时、读侧 180 秒无下行判定半开，随后按 3s→300s 退避重连。
