-- cf-panel D1 迁移 0001：初始建表（全部 IF NOT EXISTS，幂等）
-- 说明：metrics_min 的 mem_total 列由 0002 迁移追加（保持演进历史）
-- 应用：wrangler d1 migrations apply cf-panel --local/--remote
...
CREATE TABLE IF NOT EXISTS metrics_min (
  ...
  net_out   REAL,
  extra     TEXT,                            -- 扩展监控项 JSON（swap/disk/load/temp/procs/tcp/udp）
