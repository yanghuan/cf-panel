...
部署完成后访问 `https://cf-panle.<你的子域>.workers.dev`，**注册首个用户即自动成为管理员**。

本地调试：`wrangler dev --local`（本地 SQLite 建表：`wrangler d1 execute cf-panle --local --file=schema.sql`）。

## 二、添加服务器并安装 agent

1. 在面板点「添加服务器」→ 填名称 → 弹出一次性 agent 配置（WSS 地址 / UUID / KEY），**妥善保存**。
2. 在目标 Linux 机器上安装依赖：
   ```bash
...
- **多用户**：后续注册用户为普通成员，只能看到并操作自己的服务器；管理员可见全部。

## 四、API 一览

| 方法 | 路径 | 说明 |
| --- | --- | --- |
| POST | `/api/register` | 注册（首个用户为 admin） |
| POST | `/api/login` | 登录，返回 JWT |
| GET | `/api/me` | 当前用户 |
| GET | `/api/servers` | 服务器列表（按权限过滤） |
| POST | `/api/servers` | 添加服务器，返回 agent 配置 |
...
- 密码 PBKDF2-SHA256 加盐存储；agent key 只存 HMAC 哈希。
- 审计日志：`terminal.open` / `server.create` 写 `audit_logs`。
- 最小权限：member 只能操作自己的服务器。
