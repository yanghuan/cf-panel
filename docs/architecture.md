...
单实例 DO（`idFromName('metrics')`）汇聚**全部服务器**的上报与告警扫描：

- **内存热区**：每机滚动 720 分钟（分钟级聚合，每机每分钟最多 1 行），`/latest` 与监控图表查询直接命中内存，零 D1 读。
- **alarm 批量归档**：约每小时 fullSweep 一轮，从 DO Storage 恢复全量行后按 200 行/批写 D1（`metrics_min`，保留 30 天）；归档线 60 分钟、热区保留线 12 小时；归档关闭时仅按热区保留线清理。
